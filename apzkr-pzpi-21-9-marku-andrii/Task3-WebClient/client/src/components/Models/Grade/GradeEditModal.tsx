...
import { editGrade } from '../../../http/gradeApi';
...
export const GradeEditModal = ({ show, onHide, item, fetch }: IProps) => {
    const {
        control,
        handleSubmit,
        reset,
        formState: { errors },
      } = useForm<IGradeEditViewModel>();
      ...
      const onSubmit = async (data: IGradeEditViewModel) => {
        await editGrade(data.id, data)
          .then(() => {
            onHide();
            fetch();
          })
